  ...
  describe 'reading' do
    describe 'string manipulation' do
      ...
      describe 'value_chunks_from_insert(sql_statement)' do
        it 'extracts values from the insert statement' do
          ...
          expect(@reader.value_chunks_from_insert(@offices_statement).first.length).to eq(8)
          expect(@reader.value_chunks_from_insert(@offices_statement).first).to eq(["1", "San Francisco", "+1 650 219 4782", "100 Market Street", "Suite 300", "CA", "USA", "94080"])
          expect(@reader.value_chunks_from_insert(@offices_statement)[3]).to eq(["4", "Paris", "+33 14 723 4404", "43 Rue Jouffroy D'abbans", "NULL", "NULL", "France", "75017"])
    ...
    describe 'mysql ingestion' do
      ...
      it 'loads the data to the correct tables' do
        ...
        expect(@reader.database.tables.fetch('offices').rows.first.values).to eq(["1", "San Francisco", "+1 650 219 4782", "100 Market Street", "Suite 300", "CA", "USA", "94080", nil])
      end
